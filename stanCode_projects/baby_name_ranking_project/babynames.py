...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as f:
        # Don't know how to read the first line only
        filename_list = filename.split('-')
        year_list = filename_list[1].split('.')
        year = year_list[0]
        # Read every line
        for line in f:
            # Add rank and two names to a list
            ...
            if len(rank_list) > 1:
                ...
                name1 = name1.strip()
                name2 = name2.strip()
                # Use add_data_for_name function to add the data to name_data
...
def search_names(name_data, target):
    ...
    # Case-insensitive
    target.lower()
    # Empty list to record the target name
    names = []
